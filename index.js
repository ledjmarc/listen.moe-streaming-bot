...
function writeGuildConfig (guild, object) { // Change a guild's config via an object of options, and save the changes
    const currentConfig = guilds[guild] || {} // Get gurrent config for this guild, creating it if it doesn't exist
    const newConfig = merge(currentConfig, object); // Merge new options with current
    let _guilds = guilds;
...
c.once('ready', () => {
    ...
    for (let guild of Object.keys(guilds)) { // loop through all the servers recorded
        const channel = getGuildConfig(guild, 'vc'); // Get the channel for this guild
        const prefix = getGuildConfig(guild, 'prefix'); // Get the prefix for this guild
